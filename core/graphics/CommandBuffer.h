...
namespace eg {
    namespace graphics {
        ...
        struct BindDescriptorInfo
        {
            ...
            BindDescriptorInfo(uint32_t set, std::shared_ptr<DescriptorSet> ds):
                firstSet(set), ds(ds) {}
        ...
        public:
            ...
            virtual void bindDescriptorSets(uint32_t set, std::shared_ptr<DescriptorSet> ds) = 0;
            virtual void bindPipeline(const BindPipelineInfo& info) = 0;
            virtual void bindPipeline(std::shared_ptr<GraphicsPipeline> pipeline, PipelineBindPoint bindPoint = PipelineBindPoint::GRAPHICS) = 0;
            virtual void copyTexture(std::shared_ptr<Texture> srcTex, std::shared_ptr<Texture> dstTex) = 0;
            ...
            virtual void draw(const std::shared_ptr<VertexInput> vi);
            virtual void endRenderPass() = 0;
            virtual void endCommandBuffer() = 0;
            const std::shared_ptr<RenderPass> getCurrentRenderPass() const { return _currPass; }
            const std::shared_ptr<Framebuffer> getCurrentFramebuffer() const { return _currFramebuffer; }
        protected:
            ...
            std::shared_ptr<RenderPass> _currPass{ nullptr };
            std::shared_ptr<Framebuffer> _currFramebuffer{ nullptr };
        };
    }
}
